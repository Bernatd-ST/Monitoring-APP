...
<div class="card shadow">
    <div class="card-header bg-gradient-primary text-white d-flex justify-content-between align-items-center">
        <h5 class="m-0 font-weight-bold">Data Semi Finish Good</h5>
        <div>
            <button id="btn-add-semifinishgood" class="btn btn-sm btn-light me-2"><i class="fas fa-plus"></i> Tambah Data</button>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="semifinishGoodTable" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <?php if (isset($semifinish_good) && is_array($semifinish_good)): ?>
                        <?php $i = 1; ?>
                        <?php foreach ($semifinish_good as $item): ?>
                            <tr data-id="<?= $item['id'] ?>">
                                <td><?= $i++ ?></td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadExcelModalLabel">Import Data Semi Finish Good dari Excel</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('admin/ppic/upload-semifinishgood') ?>" method="post" enctype="multipart/form-data" id="uploadForm">
                    <div class="mb-3">
...
<!-- Modal Form Semi Finish Good -->
<div class="modal fade" id="semiFinishGoodModal" tabindex="-1" aria-labelledby="semiFinishGoodModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="semiFinishGoodModalLabel">Form Data Semi Finish Good</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="semiFinishGoodForm">
                    <input type="hidden" id="semiFinishGoodId" name="id">
                    ...
                    <div class="mb-3">
                        <label for="begining_bal" class="form-label">Beginning Balance</label>
                        <input type="number" class="form-control" id="begining_bal" name="begining_bal" required>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-primary" id="saveSemiFinishGoodBtn">Simpan</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="deleteSemiFinishGoodModal" tabindex="-1" aria-labelledby="deleteSemiFinishGoodModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteSemiFinishGoodModalLabel">Konfirmasi Hapus</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus data ini?</p>
                <input type="hidden" id="deleteSemiFinishGoodId">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-danger" id="confirmDeleteSemiFinishGoodBtn">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<!-- Load custom JS file -->
<script src="<?= base_url('js/ppic/semifinishgood.js') ?>"></script>
<?= $this->endSection() ?>
